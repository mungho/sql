...
=======
SELECT hd.ma_hop_dong, nv.ho_ten, kh.ho_ten, kh.so_dien_thoai, dv.ten_dich_vu, SUM(hdct.so_luong) AS so_luong_dich_vu_di_kem, SUM(tien_dat_coc) AS tien_dat_coc FROM hop_dong hd
...
AND NOT EXISTS (
SELECT * FROM hop_dong hd2 
WHERE 
hd2.ma_dich_vu = hd.ma_dich_vu
AND MONTH(hd2.ngay_lam_hop_dong) BETWEEN 1 AND 6 
...
SELECT dvdk.*, SUM(hdct.so_luong) AS so_luong FROM hop_dong_chi_tiet hdct
JOIN dich_vu_di_kem dvdk ON dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
GROUP BY dvdk.ma_dich_vu_di_kem
HAVING so_luong = (SELECT hdct2.so_luong FROM hop_dong_chi_tiet hdct2 ORDER BY hdct2.so_luong DESC LIMIT 1);

-- 14. Hiển thị thông tin tất cả các Dịch vụ đi kèm chỉ mới được sử dụng một lần duy nhất. Thông tin hiển thị bao gồm ma_hop_dong, ten_loai_dich_vu, ten_dich_vu_di_kem, so_lan_su_dung (được tính dựa trên việc count các ma_dich_vu_di_kem).
SELECT hd.ma_hop_dong, ldv.ten_loai_dich_vu, dvdk.ten_dich_vu_di_kem, 1 AS so_lan_su_dung
...
JOIN (
    ...
    HAVING COUNT(*) = 1
) AS only_once ON only_once.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
...
SELECT nv.ma_nhan_vien, nv.ho_ten, td.ten_trinh_do, bp.ten_bo_phan, nv.so_dien_thoai, nv.dia_chi, COUNT( hd.ma_hop_dong) AS so_luong_hop_dong FROM nhan_vien nv
...
>>>>>>> 528933513fee8e1e27b07ced2d8565ac920abe6c
